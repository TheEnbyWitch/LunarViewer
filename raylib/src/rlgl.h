...
unsigned int rlLoadTextureDepth(int width, int height, bool useRenderBuffer)
{
    ...
    unsigned int glInternalFormat = GL_DEPTH32F_STENCIL8;
    ...
    if (!useRenderBuffer && RLGL.ExtSupported.texDepth)
    {
        glGenTextures(1, &id);
        glBindTexture(GL_TEXTURE_2D, id);
        glTexImage2D(GL_TEXTURE_2D, 0, glInternalFormat, width, height, 0, GL_DEPTH32F_STENCIL8, GL_UNSIGNED_INT, NULL);
